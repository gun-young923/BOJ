...
이 경우, 금메달 수상자는 1번 국가의 1번 학생이고, 은메달 수상자는 1번 국가의 2번 학생이며, 
동메달 수상자는 3번 국가의 4번 학생이다. (1번 국가의 3번 학생의 성적이 동메달 수여자보다 높지만, 
...
arr = sorted(arr1, key=lambda x:-x[2])
temp = ""
for i in range(3):
    temp += str(arr[i][0])
    while temp == temp[0]*3:
