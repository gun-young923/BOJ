...
input = sys.stdin.readline
                                                                         # (int(x[3]),int(x[2]),int(x[1]))
arr = sorted([input().split() for _ in range(int(input()))], key=lambda x: [*map(int,x[3:0:-1])])
print('%s\n%s'%(arr[-1][0],arr[0][0]))                                   # [*map(int,(x[3],x[2],x[1]))]